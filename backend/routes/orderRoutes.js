...
router.post('/', verifyToken, async (req, res) => {
    ...
    try {
        const result = await db.query(
            ...
            [order_number, customer_name, customer_email || null, total_amount, user_id || null, 'Pending']
        );
...
router.put('/:id', [verifyToken, verifyAdmin], async (req, res) => {
    ...
    try {
        const result = await db.query(
            'UPDATE orders SET status=$1 WHERE id=$2 RETURNING id, order_number, customer_name as customer, total_amount as amount, status, created_at',
