    ...
    <div className="p-4 md:p-8 lg:p-12 bg-surface min-h-screen">
      <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-on-surface-variant font-body text-sm">Real-time atelier performance and critical operational alerts.</p>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <Link to="/admin/analytics" className="editorial-gradient text-white px-8 py-4 rounded-lg font-label font-bold text-[10px] uppercase tracking-widest shadow-xl hover:opacity-90 transition-all flex items-center justify-center gap-2 flex-grow md:flex-grow-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 md:gap-8 mb-12">
        {statCards.map((stat, i) => (
          <motion.div
            ...
            className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2rem] border border-outline-variant/5 shadow-2xl shadow-primary/5 group hover:border-secondary/20 transition-all cursor-pointer"
          >
            ...
            <p className="text-3xl md:text-4xl font-headline font-black text-primary tracking-tighter">{stat.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10">
        ...
        <div className="lg:col-span-8 bg-white rounded-2xl md:rounded-[2.5rem] border border-outline-variant/5 p-6 md:p-10 shadow-2xl shadow-primary/5">
            ...
            <table className="w-full text-left min-w-[500px] lg:min-w-0">
              <thead>
                <tr className="border-b border-outline-variant/5">
                  <th className="pb-5 px-2 md:px-4 font-label text-[10px] uppercase tracking-widest text-on-surface-variant font-black">ID</th>
                  <th className="pb-5 px-2 md:px-4 font-label text-[10px] uppercase tracking-widest text-on-surface-variant font-black">Customer</th>
                  <th className="pb-5 px-2 md:px-4 font-label text-[10px] uppercase tracking-widest text-on-surface-variant font-black text-right">Valuation</th>
                  <th className="pb-5 px-2 md:px-4 font-label text-[10px] uppercase tracking-widest text-on-surface-variant font-black text-center">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-outline-variant/5">
                {data?.recentActivity?.map((order) => (
                  <tr key={order.order_number} className="group hover:bg-surface/50 transition-colors">
                    <td className="py-4 px-2 md:px-4 font-label text-xs text-primary font-black">#{order.order_number}</td>
                    <td className="py-4 px-2 md:px-4 font-headline font-black text-sm text-primary">{order.customer_name}</td>
                    <td className="py-4 px-2 md:px-4 font-headline text-sm font-black text-secondary text-right">${parseFloat(order.total_amount).toFixed(2)}</td>
